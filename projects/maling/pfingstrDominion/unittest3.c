...
void checkAmbassadorEffect1(struct gameState *post) {
	...
	r = refactoredAmbassador(post, choice1, choice2, currentPlayer, handPos);
...
void checkAmbassadorEffect3(struct gameState *post) {
	...
	int r = refactoredAmbassador(post, choice1, choice2, currentPlayer, handPos);

	printf("\nTesting when choice 2 is 2: \n");
	//myAssert(r == 0, "r should be 0.");
	printf("r should be 0: %d", r);
	//myAssert(pre.handCount[currentPlayer] == post->handCount[currentPlayer] - 2, "handCount currentPlayer decrease by 2");
	printf("handcount of current player should decrease by 2.\n");
	printf("before handcount: %d", pre.handCount[currentPlayer]);
	printf("post handcount: %d", post->handCount[currentPlayer]);
